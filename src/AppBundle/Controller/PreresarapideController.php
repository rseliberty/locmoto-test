...
class PreresarapideController extends Controller
{
    ...
    public function preresarapideAction(Request $request, Vehicle $vehicle)
    {
        ...
        if($user->hasActiveRents() || !$vehicle->isAvailable() || $prerent == true || $user->getJustificatory() == 0 ){
            return $this->render('AppBundle:Preresa:preresarapide.html.twig', ['user' => $user, 'vehicle'=> $vehicle, 'prerent' => $prerent ]);
        }
        else {
            //création du formulaire de pré-réservation
            $form = $this->createForm(FormResarapideType::class, null, ['user' => $user]);
